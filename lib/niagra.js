...
function parseArguments(niagra) {
    ...
    for (var i = 0; i < process.argv.length - 1; i++) {
        ...
        default: {
            if (process.argv[i].indexOf("--app-") == 0) {
                var appoptionName = process.argv[i].substring(6)
                var appoptionValue = ""
                for (i++; i < process.argv.length - 1; i++) {
                    appoptionValue += process.argv[i]
                    if (process.argv[i + 1].indexOf("--") == 0) break
                    appoptionValue += " "
                }
                if ((parseFloat(appoptionValue) == parseInt(appoptionValue)) && !isNaN(appoptionValue)){
                    appoptionValue = parseInt(appoptionValue)
                }
                niagra.config[appoptionName] = appoptionValue
            }
        }
        }
    }
